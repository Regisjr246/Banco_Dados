...
create database if not exists construtora;
...
-- Exercicio 1: View
create view projetos_andamentos as 
select p.nome, p.descricao, p.status
from projetos p
where p.status = 'Em Andamento';

select * from projetos_andamentos pa ;
	
	
-- Exercicicio 2: Stored Procedure
	
	

	
delimiter //
create procedure inserir_funcionario(nome varchar(255), email varchar(255), senha varchar(45), id_departamento int, valor_hora decimal(15,2))
begin
	insert into funcionarios(nome, email, senha, id_departamento, valor_hora) values (nome, email, senha, id_departamento, valor_hora);
end//
delimiter ;

call inserir_funcionario('Funcionario TI', 'funcionario_t2i@empresa.com', null, 1, 50.00);
...
/* delimiter //
create procedure calcular_valor_total_projeto(p.horas_trabalhadass decimal(15,2) * p.valor_hora decimal(15,2) 
begin
	select  * from  calcular_valor_total_projeto (p.horas_trabalhadass, p.valor_hora) values (p.horas_trabalhadass, p.valor_hora);
end//
delimiter ;
call calcular_valor_total_projeto('11', 50.00);
select  * from  calcular_valor_total_projeto  ;
	*/
	...
	-- Exercicio 3 : Function
-- calcular_horas_projeto:
DELIMITER //
create function  calcular_horas_projeto(id int)
returns decimal(5,2)
begin
	declare total_horas decimal(5,2);
	select sum(horas_trabalhadas) into total_horas from funcionarios_projetos p where p.id_projeto = id ;
	return total_horas;
end //
DELIMITER ;

 select sum(horas_trabalhadas) from  funcionarios_projetos p where p.id_projeto = 1 ;
select calcular_horas_projeto(1) as horas_trabalhadas;
inner join projetos p on p.id_projeto=
-- select p.id_projeto ,p.id_projeto, calcular_horas_projeto(id) as horas_trabalhadas from projetos p ;

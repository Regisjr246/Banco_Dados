...
INSERT INTO funcionarios_projetos (id_funcionario,id_projeto,horas_trabalhadas,valor_hora) VALUES
 ...
 -- Exercicio 1
	select count( fp.id_projeto) from departamentos d
	inner join funcionarios f  on d.id_departamento = f.id_departamento
	inner join funcionarios_projetos fp  on fp.id_funcionario = f.id_funcionario 
...
-- Exercicio 3
select  f.nome as funcionario, avg(fp.horas_trabalhadas) as media     from funcionarios f 
	inner join funcionarios_projetos fp on fp.id_funcionario = f.id_funcionario 
	group by f.nome;
	...
	-- Exercicio 4
 select f.nome as fucionarios , sum(fp.horas_trabalhadas) as total_horas
 from funcionarios f 
 	inner join funcionarios_projetos fp on fp.id_funcionario = f.id_funcionario 
 	group  by f.nome
...
limit 1;
	...
	-- Exercicio 5
select  p.nome, p.dt_inicio  from projetos p 
order by dt_inicio limit 1;


	-- Exercicio 6
update projetos set valor_total =55000 where id_projeto=1;
...
where p.status = 	'Em andamento';
	...
	-- Exercicio 9
select f.nome as funcioanrio from funcionarios f 
where f.nome like '%1%';

	-- Exercicio 10
select p.nome as projetos, p.valor_total
from projetos p 
order by p.valor_total desc limit 1;
...
select p.nome as projetos, p.valor_total
from projetos p 
order by p.valor_total asc limit 1;


	-- Exercicio 12

select   p.nome as projeto, f.nome as funcionario, sum(fp.horas_trabalhadas) as horas_trabalhadas from projetos p 
 	inner join funcionarios_projetos fp on p.id_projeto = fp.id_projeto 
...
limit 5;
	...
	-- Exercicio 13
select f.nome funcionarios, sum(fp.horas_trabalhadas)as total_horas from funcionarios f 
...
select p.nome as projeto, count(fp.id_funcionario) as total_funcionario
from projetos p 
inner join funcionarios_projetos fp on p.id_projeto  =fp.id_projeto 
group by p.nome;
	...
	-- Exercicio 15
select p.nome as projeto, f.nome as funcionarios, sum(f.valor_hora * fp.horas_trabalhadas) as valor_pago 
from funcionarios f 
inner join funcionarios_projetos fp  on f.id_funcionario = fp.id_funcionario
